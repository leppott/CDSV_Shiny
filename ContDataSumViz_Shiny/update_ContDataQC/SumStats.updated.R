...
included based on user's config file."
    ...
    myFUN.sumBy <- function(x, ...){##FUN.myFUN.sumBy.START
      c(mean=mean(x,na.rm=TRUE)
        ...
        ,range=suppressWarnings(max(x,na.rm=TRUE))-suppressWarnings(min(x,na.rm=TRUE))
    ...
    df.summary <- doBy::summaryBy(x ~ Date
                                  ...
                                  , na.rm=TRUE
                                  , var.names=i)
    ## replace all NaNs and Inf to NA
    df.summary[] <- lapply(df.summary,function(x) replace(x, !is.finite(x),NA))
